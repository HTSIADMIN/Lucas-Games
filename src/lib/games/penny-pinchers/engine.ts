...
export function coinPCValue(
  ...
  // Mint Master (Boomtown) — final ×1.25/lvl multiplier on the
  // composed coin value so it scales every component proportionally.
  const mintMul = mintMasterMul(perm);
  const raw = Math.round(base * valueMul) + polishBonus + permBonus + relicBoost;
  return Math.round(raw * mintMul);
}
...
// ============================================================
// BOOMTOWN INVESTMENTS — late-game multiplier helpers
//
// Each function reads `perm` and returns a multiplier (or absolute
// value, in the case of compoundInterestPcPerSec). Callers fold the
// result into the relevant calculation — coinPCValue, applyClick,
// helperRatePcPerSec, applyPrestige, plus a few client-side hooks
// for streak / spawn / click-cascade / events.
// ============================================================

/** Mint Master — ×1.25 per level on every coin's base PC (compounds). */
export function mintMasterMul(perm: PermLevels): number {
  const lvl = perm.mint_master ?? 0;
  return lvl <= 0 ? 1 : Math.pow(1.25, lvl);
}

/** Prestige Mastery — adds +50% per level to the prestige PC multiplier. */
export function prestigeMasteryMul(perm: PermLevels): number {
  const lvl = perm.prestige_mastery ?? 0;
  return lvl <= 0 ? 1 : 1 + 0.5 * lvl;
}

/** Industrial Workforce — ×1.5 per level on helper PC/sec (compounds). */
export function industrialWorkforceMul(perm: PermLevels): number {
  const lvl = perm.industrial_workforce ?? 0;
  return lvl <= 0 ? 1 : Math.pow(1.5, lvl);
}

/**
 * Streak Conductor — amplifies the bonus *above* 1× of each streak
 * tier multiplier. So a Hot tier's 1.5 becomes 1 + 0.5 × (1 + 0.25×lvl).
 * Inputs the raw tier multiplier; outputs the boosted one.
 */
export function applyStreakConductor(tierMultiplier: number, perm: PermLevels): number {
  const lvl = perm.streak_conductor ?? 0;
  if (lvl <= 0 || tierMultiplier <= 1) return tierMultiplier;
  const bonus = tierMultiplier - 1;
  return 1 + bonus * (1 + 0.25 * lvl);
}

/** Surge Tides — additive 10% per level event start chance multiplier. */
export function surgeTidesChanceMul(perm: PermLevels): number {
  const lvl = perm.surge_tides ?? 0;
  return lvl <= 0 ? 1 : 1 + 0.1 * lvl;
}

/** Surge Tides — additive 10% per level event duration multiplier. */
export function surgeTidesDurationMul(perm: PermLevels): number {
  const lvl = perm.surge_tides ?? 0;
  return lvl <= 0 ? 1 : 1 + 0.1 * lvl;
}

/** Greedy Spawns — 3% extra-coin chance per level, capped at 60%. */
export function greedySpawnsChance(perm: PermLevels): number {
  const lvl = perm.greedy_spawns ?? 0;
  return Math.min(0.6, 0.03 * lvl);
}

/** Click Cascade — 5% chain-click chance per level, capped at 60%. */
export function clickCascadeChance(perm: PermLevels): number {
  const lvl = perm.click_cascade ?? 0;
  return Math.min(0.6, 0.05 * lvl);
}

/** Treasure Vault — ×1.25 per level on tokens awarded per prestige. */
export function treasureVaultMul(perm: PermLevels): number {
  const lvl = perm.treasure_vault ?? 0;
  return lvl <= 0 ? 1 : Math.pow(1.25, lvl);
}

/**
 * Wealth Apotheosis — lifetime ¢ banked turns into a click multiplier.
 * +1% click PC per (1M lifetime banked × level), capped at +500% per
 * level. Reads `lifetimeBankedCents` (cumulative wallet payout) so
 * the bonus persists across prestiges, unlike `lifetimePCEarned`
 * which resets on Roll It Up.
 */
export function wealthApotheosisMul(perm: PermLevels, lifetimeBankedCents: number): number {
  const lvl = perm.wealth_apotheosis ?? 0;
  if (lvl <= 0 || lifetimeBankedCents <= 0) return 1;
  const raw = 0.01 * (lifetimeBankedCents / 1_000_000) * lvl;
  const cap = 5 * lvl; // +500% per level cap
  return 1 + Math.min(cap, raw);
}

/**
 * Compound Interest — passive PC/sec generated by your Bank Token
 * reserve. Scales as √(tokens) × 50 × level so the bend is sub-linear
 * in tokens and linear in level. Returns 0 if not owned / no tokens.
 */
export function compoundInterestPcPerSec(perm: PermLevels, bankTokens: number): number {
  const lvl = perm.compound_interest ?? 0;
  if (lvl <= 0 || bankTokens <= 0) return 0;
  return Math.sqrt(bankTokens) * 50 * lvl;
}
...
export function helperRatePcPerSec(
  ...
  return rate * permBonus * relicE.helperRateMul * industrialWorkforceMul(perm);
}
...
export function permUpgradeCurrentValueLabel(id: PermUpgradeId, level: number): string | null {
  ...
  switch (id) {
    ...
    case "mint_master":
      return `×${Math.pow(1.25, level).toFixed(2)} base PC on every coin`;
    case "prestige_mastery":
      return `+${50 * level}% to prestige PC multiplier`;
    case "industrial_workforce":
      return `×${Math.pow(1.5, level).toFixed(2)} helper PC/sec`;
    case "streak_conductor":
      return `+${25 * level}% on every Pinch Streak tier`;
    case "surge_tides":
      return `+${10 * level}% event chance · +${10 * level}% duration`;
    case "greedy_spawns":
      return `${Math.min(60, 3 * level)}% chance for an extra coin per spawn`;
    case "click_cascade":
      return `${Math.min(60, 5 * level)}% chance to chain-grab a nearby coin`;
    case "compound_interest":
      return `+√(points)·${50 * level} PC/sec from your reserve`;
    case "treasure_vault":
      return `×${Math.pow(1.25, level).toFixed(2)} Prestige Points per Roll It Up`;
    case "wealth_apotheosis":
      return `+1% click PC per ${(1 / level).toFixed(2)}M lifetime ¢ banked (cap +${500 * level}%)`;
    default:
      return null;
  }
}
...
export function applyClick(
  ...
  const pc = Math.round(
    base *
      ...
      prestigeMasteryMul(state.perm) *
      wealthApotheosisMul(state.perm, state.lifetimeBankedCents) *
      relicE.clickPCMul,
  );
...
export function applyPrestige(
  ...
  const baseTokens = bankTokensFromCurrentCents(state.cents, state.prestigeCount);
  if (baseTokens <= 0) return { ok: false, error: "below_threshold" };
  // Treasure Vault (Boomtown) — multiplies the token award.
  const tokens = Math.floor(baseTokens * treasureVaultMul(state.perm));
  const now = Date.now();
  const relicE = relicEffects(state.relics);
