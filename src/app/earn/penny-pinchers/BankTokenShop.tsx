...
import { PERM_UPGRADES, type PermUpgradeDef, type PermUpgradeId } from "@/lib/games/penny-pinchers/catalog";
...
export function BankTokenShop({
  ...
  // Cheapest first, maxed perms sink to the bottom — applied per
  // section so Boomtown perms don't outrank the core foundation list.
  const sortPerms = (perms: readonly PermUpgradeDef[]) =>
    perms.slice().sort((a, b) => {
  ...
  const corePerms = sortPerms(PERM_UPGRADES.filter((p) => p.tier !== "boomtown"));
  const boomtownPerms = sortPerms(PERM_UPGRADES.filter((p) => p.tier === "boomtown"));

  const renderCard = (u: PermUpgradeDef) => {
    ...
    const isBoomtown = u.tier === "boomtown";
    return (
      <button
        key={u.id}
        type="button"
  ...
  return (
    <div className="stack pp-shop-scroll" style={{ gap: "var(--sp-2)" }}>
      ...
      {corePerms.map(renderCard)}

      {boomtownPerms.length > 0 && (
        <>
          <div
            style={{
              marginTop: "var(--sp-3)",
              paddingTop: "var(--sp-2)",
              borderTop: "2px dashed var(--gold-500)",
            }}
          >
            <div
              style={{
            ...
            >
              ★ Boomtown Investments
            </div>
            <div
              className="text-mute"
              style={{ fontSize: 10, paddingLeft: 4, marginTop: 2 }}
            >
              Late-game scaling — compound multipliers that drastically lift PC output.
            </div>
          </div>
          {boomtownPerms.map(renderCard)}
        </>
      )}
    </div>
  );
